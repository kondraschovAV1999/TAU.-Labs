...
def getStep(tf, timeLine, color, step, lable):  # Функция для получения переходной характеристики
    [y, x] = matlab.step(tf, timeLine)  # Получение переходной характеристики
    step.graph('Переходная характеристика', y, x, color, lable)


def getImpulse(tf, timeLine, color, impulse, lable):  # Функция для получения импульсной характеристики
    [y, x] = matlab.impulse(tf, timeLine)  # Получение импульсной характеристики
    impulse.graph("Импусльная характеристика", y, x, color, lable)


def getFreq(tf, timeLine, color, achx, fchx, lable):  # Функция для получения частотных характеристик
    mag, phase, omega = matlab.freqresp(tf, timeLine)  # Получение АЧХ, ФЧХ
    achx.graph("АЧХ", mag, timeLine, color, lable)
    fchx.graph("ФЧХ", phase * 180 / math.pi, timeLine, color, lable)
...
class Graph:  # Данный класс необходим для построения графиков
    ...
    def graph(self, title, y, x, color, lable):  # Функция для построения отдельного графика
        self.axes.plot(x, y, color, label=lable)  # Строится сам график уже внутри рабочей области
        plt.legend()
